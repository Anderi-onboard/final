...
/* ⭐ **盘只发给 M3,不发给 M4。** CLAUDE.md §5 写着这条、`fill.js` 的 m4 分支
   用一段注释解释它,而在此之前**没有任何东西守着它**。
   两站都发就是两个真相源,谁赢由模型当时的心情定 —— 而输的那一次没人看得见:
   解读照样写得出来,照样读着像一篇解读。给 M4 加一个 `{{board}}` 是一次单字改动,
   它不会报错、不会变慢、不会变短。**所以它得由测试来拦。** */
assert.ok(!/\{\{board\}\}/.test(M4),
  "M4 的模板里出现了 {{board}} —— 盘只发给 M3。两站都发就是两个真相源,"
  + "而它不会报错:解读照样写得出来,只是它可以绕过 M3 的取证自己读盘");
assert.ok(!/\{\{board\}\}/.test(m4) && /\{\{board\}\}/.test(read("functions/_lib/nodes/prompts.js")),
  "prompts.js 里已经没有 {{board}} 这个槽了 —— 那这条断言就退化成了一句空话");
