/* 契约:解谜管线里程序出题的那一步(functions/_lib/puzzle/)。
   ...
   ① **每条成立的判据正好变成一条线索。** 「每个模型专门解决一个判据」—— 少一条不报错,
      ...
      但必须在「打架」里带着(记冲突)。
   ② **程序读不出方向的不标。** 三合、三会成局,裁决梯模块一律判吉;可合的是忌神的五行时,
      坚牢的是伤它的那一方。这一条仓库里没有原文,程序不替它定方向。
...
import { readClueAnswer, flatCandidates } from "../functions/_lib/puzzle/program.js";
import { fillClue, fillSolve, fillM1, parseM1P } from "../functions/_lib/puzzle/prompts.js";
...
for (let s = 1; s <= N; s++) {
  ...
  for (const { m, p } of puzzles) {
    ...
    /* ② 三合、三会不继承「一律吉」 */
    for (const c of p.线索.filter((x) => x.类 === "卦形" && /^卦形\.三[合会]/.test(x.id))) {
      shapes++;
      assert.equal(c.指向, null, `${c.id} 被标了方向「${c.指向 && c.指向.向}」—— 局合的是哪一方的五行,程序定不了它帮谁`);
      assert.ok(/在这一问里是/.test(c.盘上), `${c.id} 没说合的是哪一方的五行`);
    }
...
console.log(`ok   puzzle-pipeline — ${N} 副盘 × 两问:判据线索 ${clues} 条一条不少,整条输掉的 ${lostWhole} 条都记在打架里,`
  + `三合不标方向 ${shapes} 次,用神伏藏 ${hidden} 次都说得出压在上面的是谁,候选全出自补全包`);
