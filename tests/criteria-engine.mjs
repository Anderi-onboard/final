...
/* ── ⑥ 给模型的文本里不许出现结论 ───────────────────────────────────── */
{
  ...
  assert.ok(txt.includes("这里不下结论"),
    "format() 没有告诉模型这里不下结论 —— 模型会把并列的条目读成一个判决");
  assert.ok(/判不了的/.test(format(boards[0].criteria)) || naIds.size === 0,
    "判不了的那几条没有出现在给模型的文本里 —— 沉默和否定在模型眼里一样");
