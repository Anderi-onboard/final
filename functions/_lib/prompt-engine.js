...
⚠️ **标记不改变篇幅 —— 两个方向都不改。** 这一节讲的是怎么标,既不是让你少写,也不是让你多写。
量过:刚加标记时篇幅掉到 3000 字以下(把该走的盘走短了);提醒了篇幅之后又冲到 5000 字、标了 100
多处。两次都错在同一件事上 —— **把标记当成了篇幅的旋钮**。它不是。
**篇幅照旧 3000–4000 字,标记照旧 8 到 20 处,两条线各走各的。**
超了 20 处,就在最承重的那些里面挑 20 处留下;写到 4000 字,就停。`;
