...
```
p        选下一个调色板槽位        c   给选中的槽位输入十六进制
r        让它跟着页面的色组轮换     [ ] 把循环的起点/终点设在当前帧
\        取消裁剪                  u   撤销         w   写回文件
```

⭐ **值得放在终端里的编辑只有三件,而且正好是「只能靠看才能判断」的那三件**:
这个颜色对不对、循环从哪儿开始、到哪儿结束。需要精度的事(移动一个格子、重画一个形状)
属于图像编辑器 —— 在这儿做只会做出一个很差的图像编辑器。

⚠️⚠️ **裁剪是重新编码保留的那几帧,不是把编码后的字符串数组切一刀。**
后者是显而易见的实现,而它产出的文件**打不开**:起点帧几乎总是一个增量帧,
而前面没有关键帧的增量帧解码出来什么都没有。**写出一个打不开的文件是最坏的一种「保存」**,
因为编辑器看上去照样是work的。契约因此**把写出去的每一个文件重新读回来解码**,而不是检查字节变了。

⚠️ **输入颜色时整个键盘归它。** 不然有人打 `c00`,回头发现帧步进了、程序也退了。
提示行在那一刻**整行换掉**,不是加个装饰 —— 键的含义真的变了,屏幕就得只说这件事。
`esc` 取消,`ctrl-c` 仍然直接退出(**一个出不去的模式是陷阱**)。

⚠️ **没保存的改动不会无声丢掉。** 有未保存改动时第一次按 `q` 只警告、再按一次才退;
**做了别的事之后警告会重新武装** —— 一个一直挂着的警告,会把之后那次真心实意的 `q` 变成无声的。
`ctrl-c` 不受这条管:那个键在别处就是「立刻停」,在这儿重新教它是另一种意外。

⭐ **有东西要保存的时候,保存键才排在前面。** 提示行按固定顺序删的时候,`w` 排在 `fit` 前面被删掉 ——
于是**你手上正有活要存的那一刻,唯一能存它的键不在屏幕上**。现在 `dirty` 时它的优先级升到仅次于 `quit`。

⚠️ **保存的确认信息也一样差点没了。** 提示行和消息挤不下时,原来保留提示、丢掉消息 ——
而第一个牺牲品正是「written to <file>」。**提示永远为真、永远查得到;消息是刚刚发生的事的唯一报告**,
所以挤不下时赢的是消息。
