...
1. **Apple 玻璃 UI 没落地**。你 07-24 要求"把黄色去掉,做成苹果那种玻璃 UI",我当时做了(commit `686393e`),但后来 rebase 时判断 main 的 #31 已覆盖而丢弃了它 —— 实际 #31 只恢复了 Pacifico。**现在 `.composer` 仍是暖奶油色 `rgba(244,233,210,.92)` + `saturate(1.3)`**,也就是你反复说的那个黄。要不要重做?
2. **空状态到底要什么**:问候语 + 输入框(现状),还是加回示例 chip / 介绍流?第 13 与 14 条互相矛盾。
3. **卦象排版三连**(上面 TODO 1–3)要不要这轮处理?
4. **顶栏 Claude 说明文字**:手机端已隐藏,桌面端保留 —— 可以吗?
