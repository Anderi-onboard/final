...
### 线宽 —— 按母题自己的节距定,不许写散数
`blocks.js` 里每种图案场的线宽是**该母题节距的一个分数**(tally .30 / wave .27 / scallop .21)。
绑在节距上,任何尺寸的格子里墨与地的比例都一样,改一个另几个也跟着对齐。
**低于节距的四分之一左右,笔画就不再读作"画出来的",而是读作一条淡淡的格线** —— 之前就是这样。
山景等高线、山脉等高线与摩尔纹同步加粗(山脉 2/1.55/2.45/1.9 → 2.7/2.1/3.3/2.6,摩尔纹 .85 → 1.15)。
